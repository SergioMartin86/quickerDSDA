...
rebuilt from pristine base in one command (via regenerate.sh):
  ...
  1. write an empty stub, build newTester -> capture undefined references
...
ROOT = os.path.join(QD, "source/new/core/prboom2/src")
...
def _link_output():
    """Force a clean relink of newTester and return the combined build output."""
    try:
        os.remove(os.path.join(BUILD, "newTester"))
    except OSError:
        pass
    r = subprocess.run(["ninja", "-C", BUILD, "newTester"], capture_output=True, text=True)
    return r.stdout + r.stderr


def ninja_undefined():
    """Return the set of undefined-reference symbol names for newTester.
...
def main():
    ...
    subprocess.run(["ninja", "-C", BUILD, "newTester"], capture_output=True)
    tls, kept = set(), set()
    import glob
    for o in glob.glob(os.path.join(BUILD, "newTester.p", "*.o")):
